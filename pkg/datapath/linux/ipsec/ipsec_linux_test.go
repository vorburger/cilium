...
var (
	...
	keysDat        = []byte("hmac(sha256) abcdefghijklmnopqrstuvwzyzABCDEF cbc(aes) abcdefghijklmnopqrstuvwzyzABCDEF\nhmac(sha256) abcdefghijklmnopqrstuvwzyzABCDEF cbc(aes) abcdefghijklmnopqrstuvwzyzABCDEF foobar\n")
...
func (p *IPSecSuitePrivileged) TestInvalidLoadKeys(c *C) {
	...
	c.Assert(err, IsNil)
	err = UpsertIPSecEndpoint(local, remote, spi)
	c.Assert(err, NotNil)
}
